    ...
    for (let model in models)
    {
        ...
        for (let column in attributes)
        {
            ...
            for(let property in attributes[column])
            {
                ...
                // remove custom validate
                if (property === 'validate' && typeof attributes[column][property] === 'object')
                {
                    const validates = attributes[column][property];
                    for (const name in validates)
                    {
                        if (validates.hasOwnProperty(name))
                        {
                            const element = validates[name];
                            if(typeof validates[name] === 'function')
                            {
                                delete validates[name];
                            }
                        }
                    }
                }
